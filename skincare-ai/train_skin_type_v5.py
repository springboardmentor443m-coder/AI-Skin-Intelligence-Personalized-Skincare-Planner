"""
Trains skin-type classifier v5 — using the NEW "Skin Type Identification
Research" dataset (5 classes: Combination / Dry / Normal / Oily / Sensitive).

Visual inspection confirmed this dataset contains genuine close-up skin
patches (unlike the previous dataset, which turned out to be mostly
unrelated selfies/ads). This is a real, evidence-based attempt, applying
every lesson learned from earlier rounds:
  - Class weighting from the start
  - Correct 224x224 image size
  - Early stopping + checkpointing (never lose progress on a crash)
  - Careful, limited fine-tuning

Usage:
    python train_skin_type_v5.py
"""
...
EPOCHS_PHASE1 = 25
EPOCHS_PHASE2 = 10
MODEL_OUT = "models/skin_type_model_v3.keras"  # new filename - keeps old ones safe
...
with open("models/skin_type_classes_v3.txt", "w") as f:
    f.write("\n".join(class_names))
