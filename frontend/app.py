...
def api_chat_dermatologist(user_id, message, history):
    try:
        payload = {
            "user_id": user_id,
            "message": message,
        ...
        res = requests.post(f"{BACKEND_URL}/api/chat/dermatologist", json=payload, timeout=40)
        return res.json(), res.status_code == 200
    except Exception as e:
...
def parse_routine_json(raw_routine) -> dict:
    ...
    for d in range(1, 8):
        fallback_days[f"Day {d}"] = {
            "Morning": [
                "**Step 1: Cleanse:** Wash face with a gentle hydrating cleanser.",
                "**Step 2: Treat:** Apply targeted serum for your focus area.",
                "**Step 3: Protect:** Apply broad-spectrum SPF 50 sunscreen."
            ],
            "Evening": [
                "**Step 1: Double Cleanse:** Double cleanse to remove impurities.",
                "**Step 2: Active Care:** Apply night active treatment.",
                "**Step 3: Moisture Barrier:** Lock in hydration with repair cream."
            ]
        }
...
with tab_chat:
    ...
    st.markdown("Ask any question regarding your skin concerns, ingredients, product application, or lifestyle habits. Our AI synthesizes your profile, vision ML diagnostics, and matched CSV products in real time.")

    # Top Context Summary Header
    prof = st.session_state.profile
    ...
    st.markdown(f"""
    <div style="border-radius:14px; padding:1rem 1.25rem; border:1px solid rgba(45, 106, 79, 0.2); background-color:rgba(45, 106, 79, 0.05); margin-bottom:1.25rem; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">
        <div>
            <strong style="color:#2D6A4F; font-size:1.05rem;">Active Clinical Profile:</strong>
            &nbsp; Age {prof.get('age', 25)} • {st_type} Skin • Primary Focus: <strong>{p_concern}</strong>
        </div>
        <div>
            <span class="badge-pill" style="background-color:#E8F5E9; color:#2D6A4F;">Max Budget: ${b_limit:.2f}</span>
            &nbsp;
    ...
    with chat_container:
        if not st.session_state.chat_history:
            st.markdown("""
            <div style="text-align:center; padding:2rem 1rem; opacity:0.75;">
                <h4>👋 Welcome! I am Dr. Twacha, your AI Dermatologist.</h4>
                <p>Feel free to ask me questions like:</p>
                <ul style="list-style-position: inside; text-align: left; display: inline-block;">
                    <li><em>"Why was salicylic acid recommended for my dark spots?"</em></li>
                    <li><em>"How can I adjust my routine for sensitive skin?"</em></li>
                    <li><em>"Should I apply serum before or after moisturizer?"</em></li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        else:
